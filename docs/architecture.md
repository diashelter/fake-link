...
**Estado (2026-09-18):** domínio de slug, política de destino, cifra AES-256-GCM e idempotência de criação entregues. Superfície HTTP registrada: `POST /api/v1/links` (transação, `Idempotency-Key`, `ETag` e `LinkDetail`) e consultas privadas `GET /api/v1/links` e `GET /api/v1/links/{link}` (cursor assinado, busca, estado efetivo). `PATCH`, histórico e porta de resolução ainda não.
...
**Runtime (2026-09-18):** os passos 1–3, a resposta `201` de criação e o replay idempotente estão implementados, bem como as consultas privadas `GET /api/v1/links` e `GET /api/v1/links/{link}`. Troca de destino e invalidação de cache ainda não.
