...
describe('SendPasswordResetJob', function () {
    it('sends mail to the user with the configured reset URL and ciphertext-only job payload', function () {
        Mail::fake();
        ...
        $sentinel = 'password-reset-token-sentinel-'.bin2hex(random_bytes(8));
        ...
        Mail::assertSent(PasswordResetMail::class, function (PasswordResetMail $mail) use ($user, $sentinel): bool {
            $expectedUrl = 'https://app.example.test/reset-password?token='.rawurlencode($sentinel);

            return $mail->hasTo($user->email)
                && $mail->recipientName === 'Ana Silva'
                && $mail->resetUrl === $expectedUrl
    ...
    it('does not expose the plaintext token on the job instance properties beyond ciphertext', function () {
        $sentinel = 'plain-reset-token-must-not-leak';
        $job = new SendPasswordResetJob(
    ...
    it('fails decrypt without including plaintext in the exception message', function () {
        $sentinel = 'decrypt-fail-reset-sentinel-token';
        $job = new SendPasswordResetJob(
            userId: '01901234-5678-7abc-89ab-cdef01234567',
        ...
        try {
            ...
            expect($exception->getMessage())->not->toContain($sentinel)
                ->and($exception->getMessage())->toBe('Unable to decrypt password reset token payload.');
        }
    });
});
