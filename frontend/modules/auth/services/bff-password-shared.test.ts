...
describe('loadSessionMutationContext (PW-13, PW-19)', () => {
  ...
  it('returns 403 Forbidden without loading session when Origin is invalid (PW-19)', async () => {
    const created = await createKindSession('session');
    const storeGet = vi.spyOn(store, 'get');

    const result = await loadSessionMutationContext(
  ...
  it('returns 403 Forbidden when session cookie is absent (PW-19)', async () => {
    const result = await loadSessionMutationContext(
  ...
  it('returns 403 Forbidden without a kind re-read when CSRF is invalid (PW-19)', async () => {
    const created = await createKindSession('session');
    const storeGet = vi.spyOn(store, 'get');
  ...
  it('returns 403 Forbidden when session kind is verification (PW-13)', async () => {
  ...
  it('returns session context with bearerPlaintext when kind is session (PW-13)', async () => {
    const created = await createKindSession('session');
